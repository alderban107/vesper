...
let configuredStorageFactory: ((userId: string) => CryptoStorageAdapter) | null = null

export type CryptoStorageAdapter = CryptoDbApi

export function configureCryptoStorage(
  storage: CryptoStorageAdapter | ((userId: string) => CryptoStorageAdapter)
): void {
  if (typeof storage === 'function') {
    configuredStorageFactory = storage
    _db = null
    return
  }

  configuredStorageFactory = null
  _db = storage
}
...
export function initStorage(userId: string): void {
  if (configuredStorageFactory) {
    _db = configuredStorageFactory(userId)
    return
  }
  ...
  throw new Error(
    'Crypto storage is not configured. Call configureCryptoStorage() before using the SDK outside the browser.'
  )
}
...
> {
  ...
  return results.map((r) => ({
    id: r.id,
    keyPackageRef: r.key_package_ref ?? null,
    publicData: new Uint8Array(r.key_package_public),
    privateData: new Uint8Array(r.key_package_private)
  }))
}

export async function loadKeyPackageByRef(keyPackageRef: string): Promise<{
  id: number
  keyPackageRef: string | null
  publicData: Uint8Array
  privateData: Uint8Array
} | null> {
  const result = await db().getLocalKeyPackageByRef(keyPackageRef)
  if (!result) {
    return null
  }
