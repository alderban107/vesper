...
These examples use the built SDK from `sdk/dist`.
...
node sdk/examples/cli-client.mjs search-user bob
node sdk/examples/cli-client.mjs create-conversation bob
...
node sdk/examples/mention-command-bot.mjs
```
...
VESPER_USERNAME=alice \
VESPER_PASSWORD=super-secret-password \
node sdk/examples/node-auth.mjs
```
...
node sdk/examples/node-recovery.mjs
```
...
VESPER_USERNAME=alice \
VESPER_PASSWORD=super-secret-password \
node sdk/examples/node-realtime.mjs
```
